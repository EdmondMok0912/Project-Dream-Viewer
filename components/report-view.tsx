    ...
    <div className="space-y-8 animate-in fade-in fill-mode-both duration-700">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 pb-6">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">{input.date}</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="md:col-span-1 space-y-6">
          <div className="rounded-xl bg-white p-5 shadow-sm border border-slate-200">
            ...
            <div className="space-y-4 text-sm">
              <div>
                <span className="block text-slate-500 mb-1">核心主題</span>
                <span className="font-medium text-slate-900">{report.summary.theme}</span>
              </div>
              <div>
                <span className="block text-slate-500 mb-1">核心情緒</span>
                <span className="font-medium text-slate-900">{report.summary.coreEmotion}</span>
              </div>
              <div>
                <span className="block text-slate-500 mb-1">主要象徵</span>
                <div className="flex flex-wrap gap-1 mt-1">
                  {report.summary.mainSymbols.map((s, i) => (
                    <span key={i} className="inline-flex items-center rounded-sm bg-slate-100 px-2 py-0.5 text-xs text-slate-700">
                      {s}
                    </span>
                  ))}
                </div>
              ...
              <div className="pt-2">
                <p className="text-slate-700 leading-relaxed">{report.summary.briefSummary}</p>
              </div>
            </div>
          </div>
           ...
           <div className="rounded-xl bg-white p-5 shadow-sm border border-slate-200">
            ...
            <div className="space-y-4 text-sm text-slate-700">
               {report.alternativePerspectives?.freudianView && (
                  <div>
                    <span className="block text-xs font-semibold text-slate-500 mb-1 uppercase tracking-wider">佛洛伊德觀點</span>
                    <p>{report.alternativePerspectives.freudianView}</p>
                  </div>
               )}
               {report.alternativePerspectives?.modernPsychology && (
                  <div>
                    <span className="block text-xs font-semibold text-slate-500 mb-1 uppercase tracking-wider">現代心理/認知</span>
                    <p>{report.alternativePerspectives.modernPsychology}</p>
                  </div>
               )}
               {report.alternativePerspectives?.physiologicalOrStressFactors && (
                  <div>
                    <span className="block text-xs font-semibold text-slate-500 mb-1 uppercase tracking-wider">生理與壓力因素</span>
        ...
        <div className="md:col-span-2 space-y-8">
          <section className="space-y-4">
            ...
            <div className="prose prose-slate max-w-none text-slate-700 leading-relaxed prose-p:mb-4">
               <Markdown>{report.detailedAnalysis.jungianPerspective}</Markdown>
            </div>
          </section>

          <section className="space-y-3 rounded-xl border border-slate-200 p-6 bg-white shadow-sm">
            <h3 className="text-lg font-bold text-slate-900">心理動力與意識補償</h3>
          ...
          <section className="space-y-3 rounded-xl border border-slate-200 p-6 bg-white shadow-sm">
            <h3 className="text-lg font-bold text-slate-900">現實生活映射</h3>
            <p className="text-slate-700 leading-relaxed text-sm">
              {report.detailedAnalysis.realLifeConnection}
            </p>
          </section>

          <section className="space-y-4 bg-slate-900 text-slate-50 rounded-xl p-6 shadow-md">
            <h3 className="text-lg font-bold text-white">本週反思 (Weekly Reflection)</h3>
            <ul className="space-y-3 text-sm text-slate-300 list-none p-0 m-0">
              {report.detailedAnalysis.weeklyReflectionQuestions.map((q, i) => (
                <li key={i} className="flex gap-3 items-start">
                  <span className="font-mono text-slate-500 mt-0.5 leading-relaxed">0{i + 1}</span>
